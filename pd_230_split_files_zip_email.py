...
# 文件路径设置
out_folder = 'E:/bat/output_files/split_dir/split_bd/202510'
# 创建输出目录
os.makedirs(out_folder, exist_ok=True)

source_dir = 'E:/桌面08/测试/临时拆分/拆分数据-胡国强/拆分分润数据工具/胡国强拆分文件_每月/2025-10/*.xlsx'
...
# 只处理这些BD
# target_bds = ['刘鑫', '徐建亮', '王二']  # 测试
target_bds = ['周元', '徐雅雯', '杨春艳', '曾聪', '肖伟', '张晓红', '龚建强', '李彩霞']  # 测试
...
mail_pass = os.getenv('SMTP_PASSWORD')  # 建议使用应用专用密码而非登录密码
company_name = 'SFT'

# BD邮箱映射（实际业务中可以从数据库或配置文件中读取）
# bd_emails = {
#     '刘鑫': 'liuxin05@shengpay.com',
#     # '徐建亮': 'xujianliang@shengpay.com',
#     '王二': '1649641721@qq.com'
# }
...
def send_email_with_attachment(to_email, bd_name, zip_path):
    """发送带附件的邮件函数 - 修复版本"""
    try:
        print(f"正在准备发送邮件给 {bd_name} ({to_email})...")

        with smtplib.SMTP_SSL(mail_host, mail_port, timeout=30) as sender:
            # 创建邮件对象
            ...
            msg['Subject'] = Header(f'{company_name} - {bd_name}的{month_code}数据报告', 'utf-8').encode()
...
{company_name}数据团队
            ...
            print(f"√ 成功发送邮件给 {bd_name} ({to_email})")
            return True
...
for bd_name, bd_info in bd_files_dict.items():
    ...
    if original_bd_name in bd_emails:
        ...
        send_email_with_attachment(recipient_email, original_bd_name, zip_path)
    else:
...
print(f'共生成 {len(target_bds)} 个目标BD工作簿')
